    ...
    public class FinancialAgentFactory : IFinancialAgentFactory
    {
        ...
        public async Task<AIAgent> CreateAgentAsync(string chatAiToken, string ragAiToken, bool hasPayableChatAccess, bool hasReceivableChatAccess, bool hasBankingChatAccess, string userInput = null, string userId = null)
        {
            ...
            var systemInstructions = $"""
                ...
                - MÚLTIPLAS ENTIDADES: O parâmetro `nomePessoa` aceita mais de um cliente/fornecedor separado por `|`. Exemplo: `Root Sistemas de Irrigação|Gota de Orvalho Cosméticos Orgânicos`. Quando o usuário pedir “esses dois”, “ambos”, “os dois primeiros”, “somente esses clientes” ou equivalente, você DEVE reutilizar exatamente os nomes já mostrados na resposta anterior e enviá-los juntos em `nomePessoa`. NÃO substitua por filial, empresa ou um filtro genérico.
                - DESAMBIGUAÇÃO DE EMPRESA X FILIAL: Se o usuário disser apenas `empresa`, isso pode ser ambíguo e você pode pedir esclarecimento. Mas, se ele disser `filial`, `empresa filial`, `por filial`, `nossa empresa`, `matriz` ou equivalente, trate isso diretamente como `agrupamento="FILIAL"` ou filtro de `filial`. Nesses casos, é PROIBIDO pedir desambiguação.
                ...
                - COLUNA DE DATA: Para `ConsultarContasEmAberto`, o padrão é `colunaData="DATAVENCIMENTO"`. Mas, se o usuário pedir explicitamente "por data de emissão", "agrupado por data de emissão", "emitidos em", "mês de emissão" ou equivalente, você DEVE usar `colunaData="EMISSAO"`. Para `ConsultarContasPagas`, o padrão é `DATAPAGAMENTO`, mas também deve usar `EMISSAO` quando o pedido for explicitamente por emissão.
                - AGRUPAMENTO TEMPORAL: Se o usuário pedir agrupamento por mês/ano e mencionar explicitamente qual data deve ser usada, respeite exatamente essa coluna. Exemplo: "documentos em aberto por data de emissão" => `agrupamento="MES"` com `colunaData="EMISSAO"`. NÃO troque por vencimento.
                ...
                - REFERÊNCIA À MENSAGEM ANTERIOR: Se a nova pergunta usar referências relativas como "esses dois", "os dois primeiros", "apenas os 25 e os 8", "desses clientes", "desses dois clientes", recupere do histórico imediatamente anterior quais foram as entidades exibidas e use exatamente essas entidades como filtro. Os números 25 e 8, nesse contexto, são quantidades dos grupos anteriores; eles NÃO substituem os nomes dos clientes.
